...
export function CreateBoard() {
  ...
  const { form, setForm, createBoard, cancelForm, getBoards, backgroundImages, setBackgroundImages, handleBackground, backgroundImageIndex, setBackgroundImageIndex, backgroundColorIndex, setBackgroundColorIndex, selectedBackground, setSelectedBackground } = useBoard();
  ...
  return (
    <>
      <div className={`fixed w-screen h-screen top-0 bottom-0 left-0 right-0 z-[9999] bg-[#00000070] overflow-hidden transition-opacity duration-100 ${form ? 'opacity-100 visible' : 'opacity-0 invisible'}`} />
      <section className={`fixed flex justify-center items-center w-full h-full left-0 right-0 bottom-0 top-0 z-[10000] transition-opacity duration-100 ${form ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
        <div className='h-auto w-[320px] bg-[#202520] rounded close-shadow'>
          <header className='relative grid items-center grid-cols-[32px_1fr_32px] leading-[40px] p-[20px]'>
            ...
            <button onClick={() => {reset(); cancelForm();}} className='flex col-3 row-start-1'>
              <MdCancel />
            </button>
          </header>
