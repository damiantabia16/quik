...
export const createNote = async (req, res) => {
    const { boardId } = req.params;
    const { note_title, note_content, background_color, reminder, is_archived } = req.body;
    const userId = req.user.id;
    ...
    try {
        ...
        const insert = 'INSERT INTO notes (note_title, note_content, background_color, reminder, is_archived, board_id, user_id) VALUES (?, ?, ?, ?, ?, ?, ?)';
        db.query(insert, [note_title, note_content, color, reminder, is_archived, boardId, userId], (err, result) => {
            if (err) {
